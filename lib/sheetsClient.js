...
function requestOnce(url, method, body, timeoutMs = 20000) {
  return new Promise((resolve, reject) => {
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Sheets timeout after ${timeoutMs}ms`))
    })
    req.on('error', reject)
...
export async function postSheets(webhook, data, timeoutMs = 20000) {
  const body = JSON.stringify(data)
  const first = await requestOnce(webhook, 'POST', body, timeoutMs)
  if (first.status >= 300 && first.status < 400 && first.location) {
    const loc = new URL(first.location, webhook).toString()
    const second = await requestOnce(loc, 'GET', undefined, timeoutMs)
    return parseJson(second)
  }
  return parseJson(first)
}

export async function getSheets(webhook, query, timeoutMs = 15000) {
  const url = webhook.includes('?') ? `${webhook}&${query}` : `${webhook}?${query}`
  const first = await requestOnce(url, 'GET', undefined, timeoutMs)
  if (first.status >= 300 && first.status < 400 && first.location) {
